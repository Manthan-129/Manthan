...
const PlaceOrder = () => {
  ...
  const {navigate, backendUrl, token, cartItems, setCartItems, getCartAmount, delivery_fee, products}= React.useContext(ShopContext);
  ...
  const initPay= (order)=>{
    const options= {
      ...
      handler: async (response)=>{
        try{
          const verifyResponse= await axios.post(backendUrl + '/api/order/verifyRazorpay',response,{
            headers: {Authorization: `Bearer ${token}`},
          })
          ...
          console.log("Razorpay Payment Verification Error: ", error.message);
